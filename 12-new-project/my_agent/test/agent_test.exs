defmodule AgentTest do
  ...
  test "Gets the agent with error" do
    initial = 10
    {:ok, pid} = Agent.start_link(fn -> {initial, true} end)
    {:error, _} = Agent.get(pid, fn _val -> {:error, "Ups, an explicit error"} end)
  end

  test "Updates the agent" do
    initial = 10
    {:ok, pid} = Agent.start_link(fn -> {initial, true} end)
    assert :ok == Agent.update(pid, fn {val, bool} -> {val + 1, not bool} end)
    expected = initial + 1
  ...
  test "stops the agent" do
    ...
    try do
      ...
      raise "The anget is stopped and must not get a value from state"
    catch
      :exit, _ -> nil
    end
    ...
    try do
      ...
      raise "The anget is stopped and must not update the state"
    catch
      :exit, _ -> nil
    end
  end
